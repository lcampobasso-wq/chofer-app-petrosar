const APP_CACHE  = 'chofer-app-v4';   // ← versión nueva fuerza limpieza
...
self.addEventListener('activate', e => {
  e.waitUntil(
    caches.keys().then(ks => Promise.all(
      ks.filter(k => k !== APP_CACHE && k !== TILE_CACHE).map(k => caches.delete(k))
    ))
  );
  self.clients.claim();
});
...
self.addEventListener('fetch', e => {
    ...
    e.respondWith(
      caches.open(TILE_CACHE).then(async cache => {
        ...
        try {
          ...
          return resp;
        } catch(_) { return cached || new Response('', {status:404}); }
      })
    );
    return;
  }
  ...
  // APIs externas → solo network
  e.respondWith(fetch(e.request).catch(() => new Response('', {status:503})));
});

// Pre-cache tiles desde la app
